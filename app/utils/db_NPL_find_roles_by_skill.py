...
def find_roles_for_scrum_master(min_experience=5, top_n=5, output_file="scrum_master_roles.json"):
    ...
    # example "Big Data with more than 5 years of experience in Agile methodologies, team leadership, and project delivery."
    search_text = "Senior Angular developer with more than 5 years of experience in Agile methodologies, team leadership, and project delivery."
    search_embedding = model.encode(search_text).tolist()
    search_embedding = json.dumps(search_embedding)  # Convertir el embedding a formato JSON para evitar errores
    ...
    try:
        # Ahora pasamos los 6 parámetros correctos en la consulta
        cursor.execute(query_roles, (
            search_embedding,  # Embedding convertido a JSON
            "%Scrum Master%",
            "%Scrum Master%",
            "%Scrum Master%",
            "%Scrum Master%",
